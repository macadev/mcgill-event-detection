...
def perform_request_postprocessing(video_id, youtube_url, video_attachment_path, roi_image_filename,
    ...
    text = "Hello! You requested predictions for: " + youtube_url + " You'll find the results attached!\n"
...
def create_tags(timestamps):
    delta = 0.5 # We compress timestamps that are less than half a second apart
    i = 0
    tags = []
    while i < len(timestamps) - 1:
        ...
        tags.append({'start_time': timestamps[start_index], 'end_time': timestamps[i]})
        i += 1
    return tags
...
def process_cv_engine_request(youtube_url, email, coordinates_roi, time_roi, request_type):
    ...
    try:
        ...
        db.store_generated_tags(req_id, timestamps, youtube_url, request_type)
